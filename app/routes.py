...
def post(post_id):
    post = Post.query.get_or_404(post_id) # get item or return a 404
...
@app.route("/post/<int:post_id>/delete", methods=['POST']) # only going to accept when something is submitted
@login_required
...
def reset_token(token):
    if current_user.is_authenticated:
        return redirect(url_for('home'))
    user = User.verify_reset_token(token) # the static method on the class
    if user is None:
